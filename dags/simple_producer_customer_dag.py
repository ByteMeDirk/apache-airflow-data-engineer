...
DATA_FILE_PATH = "/opt/airflow/datasets/customer_data.csv"
OUTPUT_FILE_PATH = "/opt/airflow/datasets/output_customer_data.csv"
CUSTOMER_DATASET = Dataset(OUTPUT_FILE_PATH)
...
with DAG(
    dag_id="simple_producer_customer_dag",
    description="Simple Producer DAG for Customer Data",
    start_date=days_ago(1),
    schedule_interval="@once",
    default_args=DEFAULT_ARGS,
    doc_md=DOC_MD,
    tags=["simple", "producer", "customer"],
) as dag:
    ...
    save_data = PythonOperator(
        task_id="save_data", python_callable=save_data, outlets=[CUSTOMER_DATASET]
    )
